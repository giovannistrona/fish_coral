...
norm_to_raw<-function(norm_l_,raw_l,ac=1.6){
  norm_l<-norm_l_**(1/ac)
...
cor(a$fish_sp_div,a$fe_n**3)**2
reg<-unique(a$Region)
b<-data.frame(cbind(a$phylo_div**ac,
                    a$cor_diversity,
                    a$fraction_30m,
                    a$isolation,
                    a$reef_fraction,
...
predictions_no_coral[which(predictions_no_coral<0)]<-0


raw_predictions<-norm_to_raw(predictions,a$phylo_div)
raw_predictions_no_coral<-norm_to_raw(predictions_no_coral,a$phylo_div)

afl = 1-raw_predictions_no_coral/raw_predictions

r2<-round(cor(a$phylo_div,raw_predictions)**2,2)


pdf('phylo_observed_vs_model.pdf')
max_xy<-max(c(a$phylo_div,raw_predictions))
plot(a$phylo_div,raw_predictions,
...
fits
paste('R2',r2)
paste('mean dep',mean(afl,na.rm=T))
paste('sd dep',sd(afl))
sink()
...
###data for maps
data4maps<-data.frame(cbind(a$lat,a$lon,a$cor_diversity,a$phylo_div,
                            afl*a$phylo_div,a$phylo_div-afl*a$phylo_div,afl))
...
###coral diversity theoretical scenarios
nm<-read.csv('phylo_null_model.csv',header=T)
nm$ph_d
nm_<-aggregate(nm$ph_d~nm$reef_n,FUN='mean')
...
for (rep in 1:100){
  ...
  for (cor_loss in 1:n){
    ...
    raw_predictions_no_coral<-norm_to_raw(predictions_no_coral,a$phylo_div)
    afl = 1-raw_predictions_no_coral/raw_predictions
    #afl[which(predictions==0)] = 0 not needed?
    x1<-c(x1,cor_loss)
...
for (scen in 1:3){
  ...
  for (mo in 1:n){
    ...
    raw_predictions_no_coral<-norm_to_raw(predictions_no_coral,a$phylo_div)
    afl = 1-raw_predictions_no_coral/raw_predictions
    glob_loss<-norm_to_raw(new_data$fish,a$phylo_div)*afl
    y1<-c(y1,100*(1-mean((fd0-glob_loss)/fd0)))
    fd0_nm[anom]<-nm[anom,2]
    y1_nm<-c(y1_nm,100*(mean((fd0-fd0_nm)/fd0)))
  }
